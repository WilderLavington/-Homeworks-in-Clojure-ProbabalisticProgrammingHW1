...
(defn cumsum
  ...
  (loop [results nil
    ...
    (if (seq values)
      (if results
        (recur (conj results (+ (last results) (first values)))
                      (drop 1 values))
...
(defn cumsum
  ...
  (loop [results nil
    ...
    (if (seq values)
      (if results
        (recur (concat results [(+ (last results) (first values))])
                      (drop 1 values))
        (recur (list (first values))
                      (drop 1 values))
                      )
      results)))
;; @@
...
(defn my-reduce
  ([f init values]
    (if (empty? values) 0
    (loop [results (if (empty? init) init 0)
           values values]
      (if (not (empty? values))
               (recur (f results (first values)) (rest values) )
               results)
          )
        )
      )
...
(defn my-reductions
  ([f init values]
    (lazy-seq (if (empty? values) 0
    (loop [results [(if (empty? init) init 0)]
           values values]
      (if (not (empty? values))
               (recur (conj results (f (last results) (first values))) (rest values) )
               results)
          )
        ))
      )
...
(defn numerify
  ...
  (cond
    ...
    (coll? v)
      (if (empty? v)
        (if (map? v) (into {} (map numerify v)) (map numerify v))
        v)
	:else v))
;; @@
