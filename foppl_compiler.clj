...
; f::= procedure
; e::= c | v | (let [v e1] e2) | (if e1 e2 e3) | (f e1 … en) | (c e1 … en) | (sample e) | (observe e1 e2)
; q::= e	(defn f [v1 … vn] e) q
; =>
; V::= a set of vertices V that represent random variables;
; A::= a set of arcs A in V cross V (i.e. directed edges) that represent conditional dependencies between random variables
; P::= a map P from vertices to deterministic expressions that specify the probability density or mass function for each random variable
; Y::= a partial map Y that for each observed random variable contains a pair (E, phi) consisting of a deterministic expression E for the observed  value, and a predicate expression phi that evaluates to true when this observation is on the control flow path.
; clean namespace
;namespace with required packages
(ns core
      ...
      (vector? branch)
        (cond
			...
			(= (first branch) :let)
          		...
          		; (catch Exception e (println "let rule error"  branch)))
...
; test case 2
(def foppl-code '(  (defn observe-data [data slope bias]
                    (let [xn (first data)
                          yn (second data)
                          zn (+ (* slope xn) bias)]
                      (observe (normal zn 1.0) yn)
                      (rest (rest data))))
                      (let [slope (sample (normal 0.0 11.0))
